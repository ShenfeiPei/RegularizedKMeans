...
double NetworkSimplex::regularized_f(int i, double a, double b){
    if (i==0){
        return b*b;
    }else{
        std::cout << "error: function i-th not found" << std::endl;
        return 0;
    }
}

void NetworkSimplex::Build(const std::vector<std::vector<double>>& costs, int f_th) {
  ...
  for (int i = 0; i < k_; ++i) {
    for (int j = 0; j < n_; ++j) {
      ...
      edge.in_tree = j == 0;
      edge.cap = 1;
      edge.cost = regularized_f(f_th, i, j + 1) - regularized_f(f_th, i, j);
    }
  }
  BuildTree();
}
...
void NetworkSimplex::UpdateCosts(std::vector<std::vector<double>>& costs) {
